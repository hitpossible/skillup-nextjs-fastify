...
import { getQuiz, startAttempt, submitAttempt, getLastAttempt } from "../services/quiz-service.js";
...
export async function getLastAttemptHandler(req: FastifyRequest, reply: FastifyReply) {
  const { id } = IdParam.parse(req.params);
  const result = await getLastAttempt(req.server, req.tenantId, req.userId, BigInt(id));
  return reply.status(200).send(result);
}
